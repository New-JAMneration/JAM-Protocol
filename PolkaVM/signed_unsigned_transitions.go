...
func SignedToUnsigned(a int64, n uint) (uint64, error) {
	...
	if a < -(int64(maxValue)>>1) || a >= int64(maxValue)>>1 {
		return 0, fmt.Errorf("SignedToUnsigned: input: %d out of range (%d ~ %d)", a, -(int64(maxValue) >> 1), int64(maxValue)>>1)
	}

...
pub extern "C" fn rs_encode(
    ...
    for i in 0..shard_size / 2 {
        ...
        for j in 0..data_shard {
            ...
            if encoder.add_original_shard(&shard).is_err() {
                return 2;
            }
        }
    ...
    unsafe {
        ...
        for shard_idx in 0..total_shards {
            for chunk in &chunks {
                ptr::copy_nonoverlapping(chunk[shard_idx].as_ptr(), ptr.add(offset), 2);
                offset += 2;
            }
        }
        *out_ptr = ptr;
...
pub extern "C" fn rs_decode(
    ...
    let mut output_chunks: Vec<Vec<u8>> = vec![Vec::with_capacity(shard_size); data_shards];
    ...
    for chunk_idx in 0..chunk_count {
        ...
        for (i, &shard_idx) in indices.iter().enumerate() {
            ...
            let pair = [flat[offset], flat[offset + 1]];

            if shard_idx < data_shards {
                provided[shard_idx] = Some(pair);
                if decoder.add_original_shard(shard_idx, &pair).is_err() {
                    return 2;
                }
            } else {
                let recovery_idx = shard_idx - data_shards;
                if decoder.add_recovery_shard(recovery_idx, &pair).is_err() {
                    return 2;
                }
            }
        }
        ...
        for i in 0..data_shards {
            if let Some(existing) = provided[i] {
                output_chunks[i].extend_from_slice(&existing);
            } else if let Some(recovered) = restored.get(&i) {
                output_chunks[i].extend_from_slice(recovered);
            } else {
                return 5;
            }
        }

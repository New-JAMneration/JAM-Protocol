...
func UpdateHistory() error {
	...
	if len(rhc.Betas.History) != 0 || len(rhc.Betas.Mmr.Peaks) != 0 {
		return fmt.Errorf("expected controller to have no states initially, got %+v", rhc.Betas)
	}
	...
	recent_history.STFBetaDagger2BetaPrime()
	// var (

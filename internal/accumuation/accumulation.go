...
func UpdateImmediatelyAccumulateWorkReports(reports []types.WorkReport) {
	...
	store.GetInstance().GetAccumulatedWorkReports().SetAccumulatedWorkReports(accumulatable)
}
...
func UpdateAccumulatableWorkReports() {
	...
	candidate_queue = append(candidate_queue, theta[:slot_index]...)
	candidate_queue = append(candidate_queue, store.GetInstance().GetQueuedWorkReports().GetQueuedWorkReports())
	...
	accumulated_hashes := ExtractWorkReportHashes(store.GetInstance().GetAccumulatedWorkReports().GetAccumulatedWorkReports())
	q := QueueEditingFunction(candidate_reports, accumulated_hashes)
	new_accumulatable_workreports := store.GetInstance().GetAccumulatedWorkReports().GetAccumulatedWorkReports()
	new_accumulatable_workreports = append(new_accumulatable_workreports, AccumulationPriorityQueue(q)...)
	store.GetInstance().GetAccumulatableWorkReports().SetAccumulatableWorkReports(new_accumulatable_workreports)

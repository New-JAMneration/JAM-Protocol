...
func SerializeByteSequence(input []byte) (output types.ByteSequence) {
	return WrapByteSequence(types.ByteSequence(input[:])).Serialize()
}

func SerializeOpaqueHash(input types.OpaqueHash) (output types.ByteSequence) {
...
func ExtrinsicTicketSerialization(tickets types.TicketsExtrinsic) (output types.ByteSequence) {
	/*
		(C.14) E(↕ET)
	*/
	output = append(output, SerializeU64(types.U64(len(tickets)))...)
...
func ExtrinsicGuaranteeSerialization(guarantees types.GuaranteesExtrinsic) (output types.ByteSequence) {
	/*
		(C.16) E(↕[w, E4(t), ↕a])
	...
	*/
	output = append(output, SerializeU64(types.U64(len(guarantees)))...)
	for _, guarantee := range guarantees {
		// WorkReport
		output = append(output, WorkReportSerialization(guarantee.Report)...)
		// Slot
		output = append(output, SerializeFixedLength(types.U32(guarantee.Slot), 4)...)
		// Signature
...
func ExtrinsicAssuranceSerialization(assurances types.AssurancesExtrinsic) (output types.ByteSequence) {
	...
	*/
	...
	for _, assurance := range assurances {
		...
		output = append(output, SerializeByteSequence(assurance.Bitfield.ToByteSlice())...)
...
func ExtrinsicDisputeSerialization(disputes types.DisputesExtrinsic) (output types.ByteSequence) {
	...
	*/
	// verdict
	output = append(output, SerializeU64(types.U64(len(disputes.Verdicts)))...)
	for _, verdict := range disputes.Verdicts {
		output = append(output, SerializeByteSequence(verdict.Target[:])...)
		output = append(output, SerializeFixedLength(types.U32(verdict.Age), 4)...)
		for _, judgement := range verdict.Votes {
			...
			output = append(output, SerializeFixedLength(types.U32(judgement.Index), 2)...)
			output = append(output, SerializeByteSequence(judgement.Signature[:])...)
		}
	}
	// culprit
	output = append(output, SerializeU64(types.U64(len(disputes.Culprits)))...)
	...
	// fault
	output = append(output, SerializeU64(types.U64(len(disputes.Faults)))...)
...
func RefineContextSerialization(refine_context types.RefineContext) (output types.ByteSequence) {
	...
	*/
	...
	output = append(output, SerializeFixedLength(types.U32(refine_context.LookupAnchorSlot), 4)...)
	output = append(output, SerializeU64(types.U64(len(refine_context.Prerequisites)))...)
...
func WorkPackageSpecSerialization(work_package_spec types.WorkPackageSpec) (output types.ByteSequence) {
	/*
	...
	*/
	...
	output = append(output, SerializeFixedLength(types.U32(work_package_spec.ExportsCount), 2)...)
	return output
}
...
func SerializeWorkPackage(work_package types.WorkPackage) (output types.ByteSequence) {
	/*
...
func WorkItemSerialization(work_item types.WorkItem) (output types.ByteSequence) {
	/*
	...
	*/
	...
	output = append(output, SerializeFixedLength(types.U64(work_item.Service), 4)...)
	output = append(output, SerializeOpaqueHash(work_item.CodeHash)...)
	output = append(output, SerializeU64(types.U64(len(work_item.Payload)))...)
	output = append(output, SerializeByteSequence(work_item.Payload)...)

	output = append(output, SerializeFixedLength(types.U64(work_item.RefineGasLimit), 8)...)
	output = append(output, SerializeFixedLength(types.U64(work_item.AccumulateGasLimit), 8)...)

	output = append(output, SerializeU64(types.U64(len(work_item.ImportSegments)))...)
	for _, import_spec := range work_item.ImportSegments {
		output = append(output, SerializeImportSpec(import_spec)...)
	}

	output = append(output, SerializeU64(types.U64(len(work_item.Extrinsic)))...)
	for _, extrinsic_spec := range work_item.Extrinsic {
		output = append(output, SerializeOpaqueHash(extrinsic_spec.Hash)...)
		output = append(output, SerializeFixedLength(types.U64(extrinsic_spec.Len), 4)...)
	}
	output = append(output, SerializeFixedLength(types.U64(work_item.ExportCount), 2)...)

	return output
}

func TicketBodySerialization(ticket_body types.TicketBody) (output types.ByteSequence) {
	/*
		(C.27) E(x ∈ C) ≡ E(xy, xr)
...
func SerializeImportSpec(import_spec types.ImportSpec) (output types.ByteSequence) {
	...
	output = append(output, SerializeFixedLength(types.U64(i), 2)...)
	return output
}

...
func TestEpochKeysValidate(t *testing.T) {
	...
	keys := EpochKeys(make([]BandersnatchKey, types.EpochLength))
	err := keys.Validate()
	if err != nil {
	...
	keys = EpochKeys(make([]BandersnatchKey, types.EpochLength-1))
	err = keys.Validate()
	if err == nil {
		t.Error("EpochKeys.Validate did not return an error for invalid input")
	} else if err.Error() != fmt.Sprintf("EpochKeys must have exactly %d entries, got %d", types.EpochLength, len(keys)) {
...
func TestTicketsBodiesValidate(t *testing.T) {
	...
	// Test case 1: correct number of items
	tickets := TicketsBodies(make([]TicketBody, types.EpochLength))
	...
	// Test case 2: incorrect number of items
	tickets = TicketsBodies(make([]TicketBody, types.EpochLength-1))
	err = tickets.Validate()
	if err == nil {
		t.Error("TicketsBodies.Validate did not return an error for invalid input")
	} else if err.Error() != fmt.Sprintf("TicketsBodies must have exactly %d entries, got %d", types.EpochLength, len(tickets)) {
		t.Errorf("Unexpected error message: got '%v'", err)
	}
}

func TestEpochMarkValidate(t *testing.T) {
	// Mock types.ValidatorsCount
	types.ValidatorsCount = 3

	// Test case 1: 正確的數量
	mark := EpochMark{
		Validators: make([]BandersnatchKey, types.ValidatorsCount),
	}
	err := mark.Validate()
	if err != nil {
	...
	// Test case 2: 錯誤的數量
	mark.Validators = make([]BandersnatchKey, types.ValidatorsCount-1)
	err = mark.Validate()
	if err == nil {
		t.Error("EpochMark.Validate did not return an error for invalid input")
	} else if err.Error() != fmt.Sprintf("EpochMark must have exactly %d validators, got %d", types.ValidatorsCount, len(mark.Validators)) {
		t.Errorf("Unexpected error message: got '%v'", err)
	}
}
...
func TestStateIntegrity(t *testing.T) {
	...
	} else if err.Error() != fmt.Sprintf("ValidatorsData must have exactly %d entries, got %d", types.ValidatorsCount, len(state.Lambda)) {
		t.Errorf("Unexpected error message: got '%v'", err)
	}
}

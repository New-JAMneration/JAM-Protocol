...
func Sealing(state types.State, header types.Header) {
	...
	*/
	...
	} else if len(state.GammaS.Keys) > 0 {
	...
	*/
	...
	if ep == e+1 {
		if len(state.GammaA) == types.EpochLength && int(m) >= SlotSubmissionEnd { // Z(γa) if e′ = e + 1 ∧ m ≥ Y ∧ ∣γa∣ = E
			state.GammaS.Tickets = OutsideInSequencer(&state.GammaA)
		} else { //F(η′2, κ′) otherwise
			state.GammaS.Keys = FallbackKeySequence(types.OpaqueHash(state.Eta[2]), state.Kappa)
		}
	}
}

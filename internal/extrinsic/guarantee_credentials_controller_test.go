...
func TestGuaranteeCredentialsAdd(t *testing.T) {
	...
	credential := types.ValidatorSignature{
		ValidatorIndex: 0,
	...
	expectedSig := padSignature(types.Ed25519Signature{0x5f, 0x6e, 0x74, 0xd2, 0x04, 0xc2, 0x49, 0x0e, 0x71, 0xbe, 0x44, 0x51, 0x96, 0x3d, 0x7d, 0x7d, 0xa7, 0x97, 0xd4, 0xfd, 0x37, 0xd6, 0xe0, 0xbd, 0xa5, 0x69, 0x27, 0xd0, 0x2a, 0x33, 0x02, 0xca, 0x3b, 0x3a, 0x0e, 0x08, 0xc9, 0x61, 0xe7, 0x58, 0x0e, 0x97, 0xa0, 0xf0, 0x8c, 0x26, 0x9f, 0x54, 0x97, 0x28, 0xf5, 0x2d, 0x9c, 0x7d, 0xe3, 0xaf, 0xfe, 0x85, 0x0a, 0x03, 0x71, 0x38, 0x00, 0x12}, 64)
	for i := 0; i < len(expectedSig); i++ {
		if controller.Credentials[0].Signature[i] != expectedSig[i] {
...
func TestGuaranteeCredentialsRemoveDuplicates(t *testing.T) {
	...
	credential1 := types.ValidatorSignature{
		ValidatorIndex: 0,
		Signature:      padSignature(types.Ed25519Signature{0x5f, 0x6e, 0x74, 0xd2, 0x04, 0xc2, 0x49, 0x0e, 0x71, 0xbe, 0x44, 0x51, 0x96, 0x3d, 0x7d, 0x7d}, 64),
	}
	controller.Add(credential1)

	// Add duplicate credential with same validator index but different signature
	duplicateCredential := types.ValidatorSignature{
		ValidatorIndex: 0,
		Signature:      padSignature(types.Ed25519Signature{0x3a, 0x68, 0x13, 0xf7, 0x69, 0x18, 0x95, 0xa4, 0x44, 0xd7, 0x2c, 0xad, 0x60, 0xe3, 0xd5, 0x4d}, 64),
	}
	controller.Add(duplicateCredential)

	// Add different credential
	credential2 := types.ValidatorSignature{
		ValidatorIndex: 1,
		Signature:      padSignature(types.Ed25519Signature{0xd3, 0x04, 0x51, 0x7e, 0x4d, 0xe8, 0x8a, 0x39, 0x9e, 0xd4, 0xd3, 0xfa, 0xa2, 0xfc, 0x86, 0xe3}, 64),
	}
...
func TestGuaranteeCredentialsSort(t *testing.T) {
	...
	credentials := []types.ValidatorSignature{
		{
			ValidatorIndex: 2,
			Signature:      padSignature(types.Ed25519Signature{0x02}, 64),
		},
		{
			ValidatorIndex: 0,
			Signature:      padSignature(types.Ed25519Signature{0x00}, 64),
		},
		{
			ValidatorIndex: 1,
			Signature:      padSignature(types.Ed25519Signature{0x01}, 64),
		},
	}
...
func TestGuaranteeCredentialsSet(t *testing.T) {
	...
	credentials := []types.ValidatorSignature{credential1, credential2}
	...
	controller.Set([]types.ValidatorSignature{})
	if controller.Len() != 0 {
